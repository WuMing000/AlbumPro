...
public class HorizontalDecoration extends RecyclerView.ItemDecoration {
    ...
    /*** 设置RecyclerView子视图的边距，本示例仅用于定义两个子视图之间的边距，为space*2* @param space 设置的边距*/
    public HorizontalDecoration(int space) {
        this.space = space;
    }

    /*** 获取子视图的边距* @param view 子视图* @param parent RecyclerView对象*/
    @Override
    ...
    /*** 为了使第一个和最后一个item居中，需要设置相应偏移，偏移量为RecyclerView布局宽度减去子视图的一半<P/>* 注意此处由于子视图并未实例化完成，无法通过测量得知其宽度,故需要直接获取布局宽度参数得知<P/>*/
